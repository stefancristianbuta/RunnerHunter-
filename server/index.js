...
import { JsonRpcProvider, Contract, Interface, getAddress, formatUnits } from 'ethers';
...
const FACTORIES = {
  '0x1f7d7550B1b028f7571E69A784071F0205FD2EfA'.toLowerCase(): 'Uniswap V3',
  '0x8366a39cc670b4001a1121b8f6a443a643e40951'.toLowerCase(): 'Uniswap V4',
  '0xA5aAb3F0c6EeadF30Ef1D3Eb997108E976351feB'.toLowerCase(): 'Pons V1',
  '0x7eD598BcEf8bd9Edd8C97A195C6d13f40801EC7e'.toLowerCase(): 'Pons V2'
};
const V3_POOL_TOPIC = '0x783cca1c0412dd0d695e784568c96ea0c4d9a5b9f5a5c0f6f5c5e8c0e4b6e2c';
...
async function callToken(address, method, fallback=null){
  try { const c = new Contract(address, erc20, provider); return await c[method](); } catch { return fallback; }
}
...
async function blockscoutToken(address){
  try { const r=await fetch(`${BLOCKSCOUT}/tokens/${address}`); if(!r.ok) return null; return await r.json(); } catch { return null; }
}
...
function scoreCandidate(meta, scout, trades){
  ...
  const buys=trades.filter(x=>x.buy).length, sells=trades.filter(x=>x.sell).length;
  const buyValue=trades.filter(x=>x.buy).reduce((a,x)=>a+x.value,0), sellValue=trades.filter(x=>x.sell).reduce((a,x)=>a+x.value,0);
  const total=buys+sells;
  const pressure=total ? 100*Math.min(1,(buys+1)/(sells+1)) : 0;
  ...
  const organic=Math.round((pressure*0.3)+(distribution*0.25)+(liquidity*0.25)+(Math.min(100,Math.max(0,100-Math.abs(buys-sells)/Math.max(total,1)*100))*0.2));
  const momentum=Math.min(100,activity*0.55+pressure*0.25+liquidity*0.2);
  const score=Math.round(momentum*0.55+organic*0.45);
...
async function analyzeLogs(meta,logs){
  const trades=[];
  for(const l of logs){
    if(l.address.toLowerCase()!==meta.address.toLowerCase()) continue;
    const fromAddr='0x'+l.topics[1].slice(26).toLowerCase(), toAddr='0x'+l.topics[2].slice(26).toLowerCase();
    const amount=Number(formatUnits(BigInt(l.data),meta.decimals));
    const buy=poolSet.has(fromAddr) && toAddr!==ZERO;
    const sell=poolSet.has(toAddr) && fromAddr!==ZERO;
...
async function scan(){
  ...
  for(const l of logs){ if(l.address.toLowerCase()!==WETH && l.address.toLowerCase()!==ZERO) candidates.add(l.address.toLowerCase()); }
  const results=[];
...
app.get('/api/token/:address',async(req,res)=>{
  try{
    const address=getAddress(req.params.address); const meta=await tokenMeta(address); const scout=await blockscoutToken(address); const latest=await provider.getBlockNumber(); const logs=await transferLogs(Math.max(0,latest-900),latest); const trades=await analyzeLogs(meta,logs); const metrics=scoreCandidate(meta,scout,trades); res.json({...meta,...metrics,history:history.get(address.toLowerCase())||[]});
  }catch(e){res.status(400).json({error:e.message});}
});
app.get('/api/rpc',(req,res)=>res.json({chainId:4663,rpc:RPC_URL.replace(/(api|token|key)=.*/i,'[redacted]'),publicFallback:!process.env.RH_RPC_URL,blockscout:BLOCKSCOUT}));
app.use(express.static(path.join(ROOT,'dist')));
app.get('*',(req,res)=>res.sendFile(path.join(ROOT,'dist','index.html')));
app.listen(PORT,'0.0.0.0',()=>console.log(`RunnerHunter listening on 0.0.0.0:${PORT}`));
